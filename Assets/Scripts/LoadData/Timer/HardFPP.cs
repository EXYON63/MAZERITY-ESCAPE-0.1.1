...
public class HardFPP : MonoBehaviour
{
    public Text Timer;
    public float time1;
    ...
    void Awake()
    {
        ...
        {time1 = PlayerPrefs.GetFloat("Time4"); //time1값 불러오기
        }
    }
    ...
    void Update()
    {
        ...
        {
            Timer.text = "Time : " + (Mathf.Floor(time1/60)) + ":" + (Mathf.Floor(time1 * 100)/100 - (Mathf.Floor(time1/60) * 60)); //텍스트에 적용
        }
        if(Manager.activeSelf == false)
        {
            PlayerPrefs.SetFloat("Time4", time1);
